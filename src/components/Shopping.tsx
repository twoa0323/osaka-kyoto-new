        ...
        {list.length === 0 ? (
          ...
          list.map(item => (
            <div key={item.id} className={`card-zakka bg-white flex items-center gap-4 transition-all ${item.isBought ? 'opacity-50 grayscale' : ''}`}>
              ...
              <div className="flex-1 min-w-0" onClick={() => toggleShoppingItem(trip.id, item.id)}>
                <div className="flex items-center gap-2 mb-1">
                   <span className={`text-[8px] font-black px-1.5 py-0.5 rounded text-white ${CATEGORIES[item.category].color}`}>
                     {CATEGORIES[item.category].label}
                   </span>
                   <h3 className={`font-black text-ac-brown truncate ${item.isBought ? 'line-through' : ''}`}>{item.title}</h3>
                </div>
                <p className="text-[10px] font-bold text-ac-brown/40 uppercase italic">
                  {item.price ? `${item.currency} ${item.price.toLocaleString()}` : '尚未標價'}
                </p>
              </div>

              {item.images.length > 0 && (
                <div className="w-12 h-12 rounded-lg overflow-hidden border-2 border-ac-border">
                  <img src={item.images[0]} className="w-full h-full object-cover" alt="item" />
                </div>
              )}

              <button onClick={() => deleteShoppingItem(trip.id, item.id)} className="text-ac-orange/20 hover:text-ac-orange transition-colors">
                <Trash2 size={16} />
              </button>
            </div>
          ))
            ...
            <div className="p-6 space-y-5">
              <div className="space-y-4">
                <div className="space-y-1">
                  <label className="text-[10px] font-black text-ac-brown/40 uppercase tracking-widest">商品名稱</label>
                  <input placeholder="想買什麼？" value={form.title} onChange={e => setForm({...form, title: e.target.value})} className="w-full p-4 bg-white border-2 border-ac-border rounded-2xl font-black text-ac-brown outline-none focus:border-ac-green" />
                </div>

                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-1">
                    <label className="text-[10px] font-black text-ac-brown/40 uppercase tracking-widest">預計價格</label>
                    <input type="number" placeholder="0" value={form.price || ''} onChange={e => setForm({...form, price: Number(e.target.value)})} className="w-full p-4 bg-white border-2 border-ac-border rounded-2xl font-black text-ac-brown outline-none" />
                  </div>
                  <div className="space-y-1">
                    <label className="text-[10px] font-black text-ac-brown/40 uppercase tracking-widest">分類</label>
                ...
                <div className="space-y-1">
                  ...
                  <div className="flex gap-2 overflow-x-auto py-2 hide-scrollbar">
                    <label className="min-w-[80px] h-[80px] border-4 border-dashed border-ac-border rounded-2xl flex flex-col items-center justify-center text-ac-border cursor-pointer hover:bg-white transition-colors">
                      <Camera size={20}/>
                      <input type="file" multiple accept="image/*" className="hidden" onChange={handleFileChange} />
                    </label>
                    {form.images?.map((img, i) => (
                      <div key={i} className="min-w-[80px] h-[80px] rounded-2xl overflow-hidden relative border-2 border-white">
