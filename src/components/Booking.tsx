        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-[300] flex items-center justify-center p-6" onClick={() => setDetailItem(undefined)}>
          <div className="bg-ac-bg w-full max-w-sm rounded-[40px] shadow-2xl overflow-hidden animate-in zoom-in-95" onClick={e => e.stopPropagation()}>
            <div className="p-8 space-y-6 max-h-[80vh] overflow-y-auto hide-scrollbar">
              ...
              <button onClick={() => { if(confirm('確定要永久刪除嗎？')) { deleteBookingItem(trip.id, detailItem.id); setDetailItem(undefined); } }} className="w-full py-4 text-red-400 font-bold text-[10px] uppercase tracking-widest opacity-40 hover:opacity-100 transition-opacity">Delete this item</button>
            </div>
          </div>
        </div>
      )}
...
const FlightCard = ({ item, onEdit }: any) => (
  <div className="bg-white rounded-[40px] border-4 border-ac-border shadow-zakka overflow-hidden relative active:scale-[0.98] transition-all">
    <div className="p-6 pb-2 flex justify-between items-center"><span className="text-[11px] font-black text-ac-border uppercase tracking-widest">{item.title || '航空公司'}</span><span className="bg-ac-bg text-ac-brown/30 text-[9px] px-3 py-1 rounded-full font-black uppercase">同一張訂單</span></div>
