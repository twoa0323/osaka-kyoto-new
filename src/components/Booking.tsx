  ...
  return (
    <div className="px-6 space-y-8 animate-fade-in pb-10 text-left">
      ...
      <div className="space-y-6">
        {bookings.length === 0 ? (
          <div className="text-center py-20 text-ac-border font-black italic opacity-30">尚無資訊</div>
        ) : (
          bookings.map(item => (
            <div key={item.id} className="relative cursor-pointer" onClick={() => { setEditingItem(item); setIsEditorOpen(true); }}>
              {activeSubTab === 'flight' ? (
                ...
                <FlightCard 
                  ...
                  onDelete={(e: any) => { 
                    e.stopPropagation(); 
                    if(confirm('要刪除這張機票嗎？')) deleteBookingItem(trip.id, item.id); 
                  }} 
                />
              ) : (
                /* [修正] 飯店與其他項目顯示照片並可編輯 */
                <div className="card-zakka bg-white flex justify-between items-center active:scale-[0.98] transition-all">
                  <div className="flex items-center gap-4">
                    {item.images && item.images.length > 0 ? (
                      <img src={item.images[0]} className="w-12 h-12 rounded-lg object-cover border-2 border-ac-border" alt="preview" />
                    ) : (
                      <div className="w-12 h-12 bg-ac-bg rounded-lg flex items-center justify-center text-ac-border">
                        {activeSubTab === 'hotel' ? <Home size={20}/> : activeSubTab === 'car' ? <Car size={20}/> : <Ticket size={20}/>}
                      </div>
                    )}
                    <div>
                      <h3 className="font-black text-ac-brown text-lg">{item.title}</h3>
                      <p className="text-xs text-ac-brown/40 font-bold">{item.date} • {item.confirmationNo || '無代碼'}</p>
                    </div>
                  </div>
                  <button 
                    onClick={(e) => { 
                      e.stopPropagation(); 
                      if(confirm('要刪除嗎？')) deleteBookingItem(trip.id, item.id); 
                    }} 
                    className="text-ac-orange/30 hover:text-ac-orange p-2"
                  >
                    <Trash2 size={18}/>
                  </button>
                </div>
        ...
        <button 
          ...
          className="w-full p-5 border-4 border-dashed border-ac-border rounded-[32px] text-ac-border font-black flex items-center justify-center gap-3 active:scale-95 transition-all"
        >
          <Plus /> 新增{activeSubTab === 'flight' ? '機票' : '預訂'}
        </button>
      </div>
...
const FlightCard = ({ item, onDelete }: any) => (
  <div className="bg-white rounded-[32px] border-4 border-ac-border shadow-zakka overflow-hidden active:scale-[0.98] transition-transform">
    ...
    <div className="p-6 flex justify-between items-center relative text-center">
      ...
      <div className="w-16 flex flex-col items-center px-4">
    ...
    <div className="px-6 pb-6 pt-2 border-t-4 border-dashed border-ac-bg flex justify-between items-end">
      <div>
        <p className="text-[10px] font-black text-ac-brown/30 uppercase tracking-widest">Confirmation</p>
