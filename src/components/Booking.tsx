...
import { Plane, Home, Car, Ticket, Plus, Trash2, Edit } from 'lucide-react';
        ...
        {bookings.length === 0 ? (
          ...
          bookings.map(item => (
            <div key={item.id} className="relative group cursor-pointer" onClick={() => { setEditingItem(item); setIsEditorOpen(true); }}>
              {activeSubTab === 'flight' ? (
                <FlightCard item={item} onDelete={(e) => { e.stopPropagation(); deleteBookingItem(trip.id, item.id); }} />
              ) : (
                <div className="card-zakka bg-white flex justify-between items-center active:scale-[0.98] transition-transform">
                  ...
                  <button onClick={(e) => { e.stopPropagation(); deleteBookingItem(trip.id, item.id); }} className="text-ac-orange/30 hover:text-ac-orange p-2"><Trash2 size={18}/></button>
                </div>
              )}
            </div>
          ))
...
const FlightCard = ({ item, onDelete }: { item: BookingItem, onDelete: (e: any) => void }) => (
  <div className="bg-white rounded-[32px] border-4 border-ac-border shadow-zakka overflow-hidden active:scale-[0.98] transition-transform">
    <div className="bg-ac-green p-4 flex justify-between items-center text-white">
      <div className="flex items-center gap-2"><Plane size={16} /><span className="text-[10px] font-black uppercase tracking-widest">Boarding Pass</span></div>
      <span className="font-black text-sm">{item.flightNo || 'TBD'}</span>
    </div>
    <div className="p-6 flex justify-between items-center relative">
      <div className="text-center"><h2 className="text-3xl font-black text-ac-brown">{item.depIata}</h2><p className="text-[10px] font-bold text-ac-brown/40 uppercase">{item.depTime}</p></div>
      ...
      <div className="text-center"><h2 className="text-3xl font-black text-ac-brown">{item.arrIata}</h2><p className="text-[10px] font-bold text-ac-brown/40 uppercase">{item.arrTime}</p></div>
    </div>
    <div className="px-6 pb-6 pt-2 border-t-4 border-dashed border-ac-bg flex justify-between items-end">
      <div><p className="text-[10px] font-black text-ac-brown/30 uppercase">Confirmation</p><p className="font-black text-ac-orange">{item.confirmationNo || '-'}</p></div>
      <button onClick={onDelete} className="text-ac-brown/20 hover:text-ac-orange transition-colors"><Trash2 size={16} /></button>
    </div>
  </div>
);