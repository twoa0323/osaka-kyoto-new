...
import { Plane, MapPin, Calendar, Banknote, RefreshCw, Rocket, Loader2, Mail, Lock, CheckCircle } from 'lucide-react';
...
export const Onboarding = ({ onComplete }: { onComplete: () => void }) => {
  ...
  const handleFinish = () => {
    if (!form.adminEmail || form.tripPin.length < 4) return alert("請完整填寫 Email 與 4 位密碼！🔒");
            ...
            <>
              ...
              <div className="bg-[#EDF1F7] rounded-2xl p-4">
                ...
                <div className="flex justify-between items-center border-t border-[#DDE4EE] pt-2">
                  <div className="flex items-center gap-1 text-[9px] text-[#8E99AF] font-bold"><RefreshCw size={10} className="text-ac-green" /> 匯率：每日自動刷新</div>
                  <div className="text-[10px] text-[#5C6B89] font-black">1 {form.currency} ≈ NT$ {rate.toFixed(3)}</div>
                </div>
              </div>

              <button onClick={() => { if(!form.selectedPlace) return alert("請先選擇目的地"); setStep('security'); }} className="w-full bg-[#147A70] text-white py-5 rounded-full font-black text-lg flex items-center justify-center gap-3 shadow-lg active:scale-95 transition-all mt-4"><Rocket size={24} fill="white" /> 建立行程</button>
            </>
          ) : (
            <div className="space-y-6 py-4 animate-in zoom-in-95 duration-300">
               <div className="text-center space-y-2">
                  <div className="w-16 h-16 bg-[#E2F1E7] rounded-full flex items-center justify-center mx-auto text-ac-green"><Lock size={32}/></div>
                  <h3 className="text-xl font-black text-ac-brown italic">設定行程存取密碼</h3>
                  <p className="text-xs text-ac-brown/50 font-bold">未來切換至此行程時需輸入密碼進入</p>
               </div>
               <div className="space-y-4">
                  <div className="bg-[#EDF1F7] rounded-2xl p-4 flex items-center gap-4"><Mail className="text-[#5C6B89]" size={20}/><input placeholder="Email (忘記密碼用)" className="bg-transparent w-full text-ac-brown font-black outline-none" value={form.adminEmail} onChange={e => setForm({...form, adminEmail: e.target.value})} /></div>
               ...
               <div className="flex gap-3">
                  <button onClick={() => setStep('info')} className="flex-1 py-4 border-2 border-ac-border rounded-full font-black text-ac-border">上一步</button>
                  <button onClick={handleFinish} className="flex-[2] bg-ac-green text-white py-4 rounded-full font-black shadow-zakka">確認並啟航 ➔</button>
               </div>
            </div>
          )}
        </div>
