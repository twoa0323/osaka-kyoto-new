...
import { Wallet, Coins, MapPin, Image as ImageIcon, Trash2, Camera, X, Search, Edit3, BarChart3, ScanLine, Upload, PenTool, LayoutList, Settings, CheckCircle } from 'lucide-react';
  ...
  useEffect(() => {
    if (trip?.baseCurrency && trip.baseCurrency !== 'TWD') {
      fetch(`https://open.er-api.com/v6/latest/${trip.baseCurrency}`)
        ...
        .catch(() => setExchangeRate(0.21)); // 失敗時的備用 (日幣)
    } else {
      setExchangeRate(1);
    }
        ...
        <div className="space-y-6 animate-in slide-in-from-right">
          <div className="card-zakka bg-[#1A1A1A] text-white border-none p-6 space-y-4 shadow-xl">
            <div className="flex justify-between items-start">
              <div><h3 className="font-black text-lg">預算與剩餘</h3><p className="text-xs opacity-50 uppercase tracking-widest">Budget Status</p></div>
          ...
          <div className="card-zakka bg-white border-4 border-ac-border p-8 text-center shadow-zakka">
            <h3 className="font-black text-ac-brown text-left mb-6 flex items-center gap-2"><div className="w-1 h-4 bg-ac-orange rounded-full"/> 支出類別統計</h3>
            <DonutChart data={pieData} totalLabel="Categories" />
            <div className="grid grid-cols-2 gap-3 mt-8 text-left">
              {pieData.map(d => <div key={d.label} className="flex items-center gap-2 text-xs font-bold text-ac-brown"><div className="w-2 h-2 rounded-full" style={{background: d.color}}/> {d.label} <span className="opacity-30 ml-auto">${Math.round(d.value).toLocaleString()}</span></div>)}
            </div>
          </div>
          <button onClick={() => setActiveTab('record')} className="w-full py-4 text-center text-ac-border font-black text-[10px] uppercase tracking-widest">Back to record</button>
        </div>
      ) : (
        <>
            ...
            <div className="card-zakka bg-white p-6 space-y-6 animate-in fade-in relative overflow-hidden shadow-zakka">
              ...
              {(inputMode === 'scan' || inputMode === 'import') && (
                <div className="border-4 border-dashed border-ac-border rounded-[32px] p-10 text-center space-y-4 bg-ac-bg">
                  ...
                  <><p className="text-ac-brown font-bold text-xs">拍攝收據 AI 會自動帶入唷！</p><button onClick={() => aiInputRef.current?.click()} className="btn-zakka px-8 py-3">{inputMode === 'scan' ? '開啟相機 📸' : '選擇照片 🖼️'}</button>
              ...
              <div className={`space-y-5 transition-all ${inputMode !== 'manual' ? 'opacity-30 pointer-events-none' : ''}`}>
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="space-y-1"><label className="text-[10px] font-black text-ac-brown/40 uppercase">幣別</label><div className="flex gap-2">{[trip.baseCurrency, 'TWD'].map(c => <button key={c} onClick={() => setForm({...form, currency: c as any})} className={`flex-1 h-[62px] rounded-xl font-black border-2 ${form.currency === c ? 'bg-[#E2F1E7] border-ac-green text-ac-green shadow-sm' : 'bg-white border-ac-border text-ac-border'}`}>{c}</button>)}</div></div>
                </div>

                <div className="space-y-1"><label className="text-[10px] font-black text-ac-brown/40 uppercase">支付方式</label>
                <div className="flex gap-2">{['現金','信用卡','行動支付'].map(m => <button key={m} onClick={() => setForm({...form, method: m as any})} className={`flex-1 py-3 rounded-xl font-black text-xs border-2 ${form.method === m ? 'bg-ac-orange text-white border-ac-orange' : 'bg-white border-ac-border text-ac-border'}`}>{m}</button>)}</div></div>

                <div className="space-y-1"><label className="text-[10px] font-black text-ac-brown/40 uppercase">地點 (Google Maps)</label>
                ...
                <button onClick={handleSave} className="btn-zakka w-full py-5 text-xl">{editingId ? '確認更新 ➔' : '儲存紀錄 ➔'}</button>
              </div>
            </div>
          )}
              ...
              {Object.keys(grouped).sort((a,b) => b.localeCompare(a)).map(date => {
                ...
                return (
                  <div key={date} className="space-y-3">
                    ...
                    {grouped[date].map(e => (
                      <div key={e.id} onClick={() => handleEdit(e)} className="card-zakka bg-white flex justify-between items-center group active:scale-95 transition-all shadow-sm">
