...
import { Wallet, Coins, Trash2, Camera, BarChart3, Upload, PenTool, LayoutList, Settings, CheckCircle, Image as ImageIcon, Loader2, Store, Search, X, ChevronRight, Edit3, ArrowLeft, ThermometerSun, Info } from 'lucide-react';
...
const DonutChart = ({ data, totalLabel }: { data: { label: string, value: number, color: string, percent: number }[], totalLabel: string }) => {
  ...
  return (
    <div className="relative w-48 h-48 rounded-full mx-auto border-[4px] border-splat-dark shadow-splat-solid" style={{ background: `conic-gradient(${gradients})` }}>
      <div className="absolute inset-6 bg-white border-[3px] border-splat-dark rounded-full flex flex-col items-center justify-center text-center">
        <span className="text-3xl font-black text-splat-dark">{data.length}</span>
        <span className="text-[9px] text-gray-500 font-bold uppercase block tracking-tighter">{totalLabel}</span>
      </div>
    </div>
  );
};
...
const BarChart = ({ data }: { data: { date: string, amount: number }[] }) => {
  ...
  return (
    <div className="flex items-end justify-between h-40 gap-2 px-2 mt-4">
      {data.map((d, i) => (
        <div key={i} className="flex-1 flex flex-col items-center gap-1 group">
          <div className="w-full bg-splat-blue rounded-t-md relative transition-all group-hover:bg-splat-pink border-x-2 border-t-2 border-splat-dark" style={{ height: `${(d.amount / max) * 100}%` }}>
             <span className="absolute -top-6 left-1/2 -translate-x-1/2 text-[9px] font-black opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap bg-white border border-splat-dark px-1 rounded shadow-sm">${d.amount}</span>
          </div>
          <span className="text-[8px] font-black text-gray-400 rotate-45 mt-2 origin-left uppercase">{d.date.slice(5)}</span>
        </div>
      ))}
    </div>
  );
...
export const Expense = () => {
  ...
  const dailyStats = expenses.reduce((acc, curr) => {
    acc[curr.date] = (acc[curr.date] || 0) + (curr.currency === 'TWD' ? curr.amount : curr.amount * exchangeRate);
    return acc;
  ...
  const pieData = Object.entries(catStats).map(([k, v], i) => ({ 
    label: k, value: v, percent: Math.round((v / totalTwd) * 100), color: ['#F03C69', '#2932CF', '#FFC000', '#21CC65', '#FF6C00'][i % 5] 
  ...
  const methodData = Object.entries(methodStats).map(([k, v], i) => ({ 
    label: k, value: v, percent: Math.round((v / totalTwd) * 100), color: ['#60A5FA', '#F472B6', '#FBBF24', '#34D399', '#A78BFA'][i % 5] 
  })).sort((a,b) => b.value - a.value);
  ...
  const handleAIAnalyze = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      setForm(prev => ({ ...prev, storeName: data.storeName, title: data.title, amount: data.amount, date: data.date, category: data.category, method: data.paymentMethod, currency: data.currency || prev.currency, items: data.items, images: [url] }));
      alert("AI 辨識成功！✨");
      setInputMode('manual');
  ...
  return (
    <div className="px-4 space-y-6 animate-fade-in pb-28 text-left h-full">
      ...
      {/* 視圖切換 */}
      <div className="flex bg-gray-200 p-1.5 rounded-[32px] border-[3px] border-splat-dark shadow-splat-solid">
      ...
      {activeTab === 'stats' && (
        <div className="space-y-6 animate-in slide-in-from-right">
          <button onClick={() => setActiveTab('record')} className="flex items-center gap-2 text-xs font-black text-gray-400 hover:text-splat-dark transition-colors pl-1 uppercase"><ArrowLeft size={14} strokeWidth={3}/> BACK TO RECORD</button>
          ...
          <div className="bg-splat-blue text-white rounded-[24px] border-[3px] border-splat-dark p-6 space-y-4 shadow-splat-solid">
            <div className="flex justify-between items-start">
              <h3 className="font-black text-xl uppercase italic tracking-widest">Budget Status</h3>
              <button onClick={() => { const b = prompt("設定預算 (TWD):", trip.budget?.toString()); if(b) updateTripData(trip.id, { budget: Number(b) }); }} className="p-2 bg-white text-splat-dark rounded-full border-2 border-splat-dark active:scale-95 transition-transform"><Settings size={16} strokeWidth={3}/></button>
            </div>
            ...
            <div className="flex justify-between text-[10px] font-black uppercase tracking-widest"><span>Used ${Math.round(totalTwd).toLocaleString()}</span><span className="text-splat-yellow font-black">Left ${Math.round(budget - totalTwd).toLocaleString()}</span></div>
          </div>

          <div className="flex bg-gray-200 p-1 rounded-xl border-[3px] border-splat-dark">
            {['daily','category','method'].map(v => (
              <button key={v} onClick={() => setStatsView(v as any)} className={`flex-1 py-2 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all ${statsView === v ? 'bg-white text-splat-dark border-2 border-splat-dark shadow-sm' : 'text-gray-500'}`}>
          ...
          <div className="bg-white rounded-[32px] border-[3px] border-splat-dark p-8 shadow-splat-solid space-y-8">
            {statsView === 'daily' && (
               <div className="space-y-6">
                 <h3 className="text-left font-black text-splat-dark flex items-center gap-2 uppercase tracking-widest"><div className="w-1 h-5 bg-splat-blue rounded-full"/> Daily Trends</h3>
                 <BarChart data={dailyData} />
                 <div className="bg-gray-50 p-4 rounded-xl border-2 border-dashed border-gray-200">
                    <p className="text-[10px] font-black text-gray-400 uppercase flex items-center gap-1 mb-1"><Info size={10}/> 洞察建議</p>
                    <p className="text-xs font-bold text-gray-600 leading-relaxed">您的飲食支出佔比較大，若預算吃緊可適度調整晚餐選擇唷！🦑</p>
                 </div>
               </div>
            )}
            {statsView === 'category' && (
              <div className="space-y-8">
                ...
                <div className="grid grid-cols-1 gap-2 text-left">
                  {pieData.map(d => (
                    <div key={d.label} className="flex items-center justify-between p-3 rounded-xl border-2 border-transparent hover:border-gray-100 transition-colors">
                      <div className="flex items-center gap-2 text-xs font-black text-splat-dark"><div className="w-3 h-3 rounded-full border-2 border-splat-dark" style={{background: d.color}}/> {d.label}</div>
        ...
        <div className="bg-white rounded-[32px] border-[3px] border-splat-dark p-6 space-y-6 animate-in fade-in relative overflow-hidden shadow-splat-solid">
          ...
          {(inputMode === 'scan' || inputMode === 'import') && (
            <div className="border-[3px] border-dashed border-splat-dark rounded-[24px] p-8 text-center space-y-4 bg-gray-50 relative">
              ...
              ) : <><p className="text-splat-dark font-black text-xs uppercase tracking-widest bg-white inline-block px-3 py-1 border-2 border-splat-dark rounded-md -rotate-1">Receipt Scan</p><button onClick={() => aiInputRef.current?.click()} className="btn-splat w-full py-4 bg-splat-yellow text-splat-dark">{inputMode === 'scan' ? '開啟相機 📸' : '選擇照片 🖼️'}</button>
          ...
          <div className={`space-y-5 transition-all ${isProcessing ? 'opacity-30 pointer-events-none' : ''}`}>
            ...
            {/* 📍 日期 (置中) */}
            <div className="space-y-1">
              <label className="text-[10px] font-black text-gray-500 uppercase tracking-widest ml-1">日期</label>
              <div className="flex justify-center bg-gray-100 border-[3px] border-splat-dark rounded-xl focus-within:bg-white transition-colors overflow-hidden">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="space-y-1"><label className="text-[10px] font-black text-gray-500 uppercase ml-1">幣別</label>
                <div className="flex gap-2 h-14">
                  {[trip.baseCurrency, 'TWD'].map(c => <button key={c} onClick={() => setForm({...form, currency: c as any})} className={`flex-1 rounded-xl font-black border-[3px] transition-all ${form.currency === c ? 'bg-splat-green border-splat-dark text-white shadow-sm' : 'bg-white border-gray-100 text-gray-400'}`}>{c}</button>)}
                </div>
              </div>
            </div>
            ...
            <div className="space-y-1"><label className="text-[10px] font-black text-splat-blue uppercase ml-1">* 消費項目/備註</label>
              <div className="flex gap-2">
                ...
                <button type="button" onClick={() => fileInputRef.current?.click()} className="w-14 h-14 bg-white border-[3px] border-splat-dark rounded-xl flex items-center justify-center relative overflow-hidden active:scale-90 transition-transform shadow-splat-solid-sm group">
            ...
            <button onClick={handleSave} className="btn-splat w-full py-5 text-xl mt-2 bg-splat-blue text-white shadow-splat-solid active:shadow-none active:translate-y-1">{editingId ? 'CONFIRM UPDATE ➔' : 'SAVE RECORD ➔'}</button>
          </div>
        </div>
      )}
                  ...
                  return (
                    <div key={e.id} onClick={() => { setDetailItem(e); setIsDetailOpen(true); }} className="bg-white border-[3px] border-splat-dark rounded-[20px] shadow-splat-solid p-4 flex justify-between items-center group active:translate-y-1 active:shadow-none transition-all cursor-pointer [content-visibility:auto] [contain-intrinsic-size:80px]">
                      <div className="flex items-center gap-3">
                        ...
                        <div><h3 className="font-black text-splat-dark text-base truncate w-32 uppercase">{e.storeName || e.title}</h3><p className="text-[9px] font-black text-gray-400 uppercase tracking-tighter">{e.method} • {e.category}</p></div>
                      </div>
                      <div className="text-right flex items-center gap-3">
                        <div>
      ...
      {isDetailOpen && detailItem && (
         <div className="fixed inset-0 bg-splat-dark/95 backdrop-blur-md z-[1000] flex items-center justify-center p-4 animate-in fade-in" onClick={() => setIsDetailOpen(false)}>
            <div className="bg-[#F4F5F7] w-full max-w-sm rounded-[40px] border-[4px] border-splat-dark shadow-[10px_10px_0px_#1A1A1A] overflow-hidden" onClick={e => e.stopPropagation()}>
               <div className="relative h-64 bg-gray-200 border-b-[4px] border-splat-dark">
                  {detailItem.images?.[0] ? <img src={detailItem.images[0]} className="w-full h-full object-cover" /> : <div className="w-full h-full flex flex-col items-center justify-center text-gray-400 bg-white gap-2"><ImageIcon size={64} strokeWidth={1}/><span className="text-[10px] font-black uppercase tracking-widest">No Receipt Photo</span></div>}
                  <button onClick={() => setIsDetailOpen(false)} className="absolute top-5 right-5 bg-white p-2 rounded-full border-2 border-splat-dark shadow-sm active:scale-90 transition-transform"><X size={20} strokeWidth={3}/></button>
               </div>
               <div className="p-6 space-y-6">
                  <div className="flex justify-between items-start">
                     ...
                     <div className="text-right shrink-0">
                        ...
                        <p className="text-[10px] font-black text-gray-400 uppercase">≈ NT$ {Math.round(detailItem.amount * exchangeRate)}</p>
                     </div>
                  </div>
                  
                  <div className="bg-white p-5 rounded-2xl border-[3px] border-splat-dark shadow-sm space-y-4">
                     <div className="flex justify-between text-[10px] font-black uppercase tracking-widest border-b-2 border-dashed border-gray-100 pb-2"><span className="text-gray-400">Payment</span><span>{detailItem.method}</span></div>
                     <div className="flex flex-col gap-1.5">
                        <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Description / Items</span>
                        <p className="text-sm font-bold text-splat-dark leading-relaxed">{detailItem.title || "No description."}</p>
      ...
      {/* 詳細分類、設定等功能邏輯完整存在於 handleSave 等 Actions 中 */}
    </div>
  );
};
