...
export const Expense = () => {
  ...
  const handleSave = () => {
    if (!form.title || !form.amount) return alert("請填入內容與金額唷！💰");
    ...
    <div className="px-6 space-y-6 animate-fade-in pb-10 text-left">
      ...
      {viewMode === 'input' ? (
        ...
        <div className="space-y-4">
          {(trip.expenses || []).length === 0 ? <div className="text-center py-20 text-ac-border font-black italic opacity-30">尚無紀錄唷！🏮</div> :
            [...trip.expenses].reverse().map(e => (
              <div key={e.id} onClick={() => { setForm(e); setEditingId(e.id); setViewMode('input'); }} className="card-zakka bg-white flex justify-between items-center group cursor-pointer active:scale-95 transition-all">
                <div className="flex items-center gap-4">
                  {e.images?.[0] ? <img src={e.images[0]} className="w-10 h-10 rounded-full object-cover border-2 border-ac-bg" /> : <div className="w-10 h-10 bg-ac-bg rounded-full flex items-center justify-center text-ac-orange"><Coins size={20}/></div>}
