      ...
      <div className="grid grid-cols-2 gap-4">
        {(trip.journals || []).map(item => (
          <div key={item.id} className="bg-white p-2 pb-4 rounded-xl shadow-zakka border-2 border-ac-border rotate-[-1deg] hover:rotate-0 transition-transform group relative">
             ...
             <button onClick={() => deleteJournalItem(trip.id, item.id)} className="absolute top-1 right-1 p-1 bg-white/80 rounded-full opacity-0 group-hover:opacity-100 text-ac-orange transition-opacity"><Trash2 size={12}/></button>
          </div>
        ))}
      </div>
          ...
          <div className="bg-ac-bg w-full max-w-md rounded-t-[40px] sm:rounded-[40px] shadow-2xl overflow-hidden animate-in slide-in-from-bottom-10 max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-6 space-y-5">
              ...
              <div className="flex gap-2 overflow-x-auto py-2 hide-scrollbar">
                <label className="min-w-[100px] h-[100px] border-4 border-dashed border-ac-border rounded-2xl flex flex-col items-center justify-center text-ac-border cursor-pointer hover:bg-white transition-colors">
                  <Camera size={24}/>
                  <span className="text-[10px] font-black">拍照/上傳</span>
