...
export const Schedule = () => {
  ...
  const handleAiAnalyze = async () => {
    ...
    if (!aiText.trim()) return alert("請貼入內容唷！");
    
    setIsAiLoading(true);
    try {
      const genAI = new GoogleGenerativeAI(GEMINI_API_KEY);
      // 使用 Gemini 3 Flash preview 預覽版 (目前效能最頂尖的版本)
  ...
  return (
    <div className="space-y-6 animate-fade-in pb-10">
      <div className="px-6 flex gap-4">
        ...
        <div className="card-zakka flex-1 flex flex-col items-center justify-center py-4 bg-white text-ac-brown"><Sun className="text-ac-orange mb-1" size={24} /><span className="text-lg font-black italic text-ac-brown">24°C</span></div>
      </div>
      ...
      <div className="px-6 space-y-6 relative text-left">
              ...
              <div key={item.id} className="flex gap-4 items-start relative group">
                ...
                <div className="card-zakka flex-1 active:scale-[0.98] transition-transform cursor-pointer">
                  <div className="flex justify-between items-start">
                    ...
                    <button onClick={() => deleteScheduleItem(trip.id, item.id)} className="text-ac-orange/40 hover:text-ac-orange transition-colors"><Trash2 size={16} /></button>
                  </div>
        ...
        <button onClick={() => setIsEditorOpen(true)} className="flex items-center gap-3 w-[calc(100%-48px)] p-5 border-4 border-dashed border-ac-border rounded-[32px] text-ac-border font-black ml-12 active:scale-95 transition-all"><Plus size={20} /> 新增手寫項目</button>
      </div>
      ...
      {isAiOpen && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[600] flex items-center justify-center p-6 animate-in fade-in">
          <div className="bg-ac-bg w-full max-w-md rounded-[40px] shadow-2xl p-8 space-y-6 animate-in zoom-in-95">
            <div className="flex justify-between items-center"><h2 className="text-xl font-black text-purple-600 flex items-center gap-2"><Sparkles/> AI 智慧導入</h2><button onClick={() => setIsAiOpen(false)} className="p-2 bg-white rounded-full"><X size={16}/></button></div>
            <textarea placeholder="例如：下午兩點去黑門市場吃章魚燒..." className="w-full h-48 bg-white border-4 border-ac-border rounded-3xl p-4 font-bold text-ac-brown outline-none focus:border-purple-400 resize-none shadow-inner" value={aiText} onChange={e => setAiText(e.target.value)} />
            <button onClick={handleAiAnalyze} disabled={isAiLoading} className="w-full bg-purple-500 text-white py-5 rounded-full font-black shadow-zakka active:scale-95 flex items-center justify-center gap-3 transition-all">
              {isAiLoading ? <Loader2 className="animate-spin" /> : "開始智慧解析 ➔"}
            </button>
          </div>
        </div>
      )}
